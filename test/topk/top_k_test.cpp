...
int main() {
    ...
    // Plaintext plain_half_one;
    // ckks->encoder->encode(init_vec_with_value(x.size(), 0.5),score.parms_id(), score.scale(), plain_half_one);
    // ckks->evaluator->multiply_plain_inplace(score, plain_half_one);
    ckks->evaluator->multiply(score, ct, relu_x);


    time_end = high_resolution_clock::now();
    cout << "SGN took " << duration_cast<milliseconds>(time_end - time_start).count()/2 << " ms" << endl; //TODO: HEXL Acc
