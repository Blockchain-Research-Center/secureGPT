...
int CKKSManager::poly_modulus_degree()
{
    return N;
}

int CKKSManager::real_number_capacity()
{
    return N / 2;
}
...
void CKKSManager::init_ckks(bool VERBOSE)
{
    ...
    if (VERBOSE)
        cout << "nb_bits: " << nb_bits << endl;
    if (VERBOSE)
        cout << "Chose N=" << N << " so max bit count is " << CoeffModulus::MaxBitCount(N) << endl;
    if (VERBOSE)
        cout << "Will attempt to use " << 60 + nb_bits * depth + 60 << " bits (depth = " << depth << ")" << endl;
    ...
    // Set up tools
    encryption_parameters = make_shared<EncryptionParameters>(parms);
